...
[assistant] The quote checks out verbatim. Now let me probe the technical substance — whether "advertises tool-calling" actually means the endpoint executes anything.

[assistant] Key technical finding. Let me verify how the report frames exploitation (observed vs. assessed) and check secondary coverage for qualification.

[assistant] Verification complete. The number is real; the causal claim built on it does not survive.