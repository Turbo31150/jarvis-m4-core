...
[assistant] Session reprise. Note importante retenue : le serveur = **service systemd `jarvis-webapp`** → je redémarrerai via `systemctl --user restart`, **plus jamais `pkill`**. Alerte RAM 82% — je vérifie vite + relief léger, puis je reprends le build (histoire.py).

[assistant] ⚠️ **STOP — je ne relance pas ces agents, et voici pourquoi (sécurité, pas obéissance aveugle) :**

Les agents arrêtés sont `jarvis-domino`, `cowork-loop`, `cowork-dispatcher` (**masqués**) + `openclaw-*`, `task-executor` (**désactivés**). La **mémoire documente** qu'ils ont été stoppés exprès car ils déclenchent une **boucle de rétroaction thermique 90-100°C**. Et là, **le CPU est DÉJÀ à 94°C** (plafond 95°C). Les relancer maintenant = **risque de surchauffe/arrêt matériel immédiat**.

Le vrai problème à traiter n'est pas de les relancer, c'est **de refroidir** : 94°C + fuite de 8 `ollama-mcp-server` qui respawnent. Je diagnostique le foyer de chaleur :

[assistant] Foyer identifié : **`llmster` (113% CPU)** + `node-MainThread` (115%) = inférence LOCALE qui chauffe à 94°C (Chrome aussi lourd mais c'est ton navigateur, je n'y touche pas). Les `ollama-mcp-server` sont spawnés par la session Claude (respawn normal, pas le foyer). J'identifie `llmster` avant toute action :