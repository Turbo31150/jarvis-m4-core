...
**Objectif mesurable** : Implémenter des connecteurs robustes pour au moins trois exchanges (Binance, Kraken, Coinbase) et produire un flux normalisé conforme au schéma TradeOracle en moins de 200 ms de latence.  
**Notions couvertes**  
...
**Objectif mesurable** : Développer un pipeline d’agrégation qui combine au moins deux modèles de prévision (LSTM et Gradient Boosting) et un algorithme de consensus (Weighted Majority Voting) pour produire une prédiction de prix avec un RMSE inférieur à 2 % sur un jeu de test de 30 jours.  
**Notions couvertes**  
...
**Objectif mesurable** : Implémenter et back‑tester deux stratégies (mean‑reversion et breakout) qui utilisent les signaux du moteur de consensus, et obtenir un ratio gain/perte (Profit Factor) supérieur à 1,5 sur les données historiques de 2022‑2023.  
**Notions couvertes**  
...
- Le multi‑stage élimine le compilateur du runtime, limitant la taille de l’image à < 80 Mo.  
...
| **Timestamp synchronizer** | Aligne les timestamps avec le temps UTC du cluster | `ntplib` + correction de drift (ex. `offset = server_time - local_time`) | NTP fallback, dérive > 50 ms → rejet du message |
    ...
    async with connect(url, ping_interval=20, ping_timeout=10) as ws:
        ...
        async for raw_msg in ws:
            try:
                ...
                if abs(drift) > 50:                     # > 50 ms → désynchronisation
                    logging.warning("Drift %d ms, discarding", drift)
                    continue
                await queue.put(trade)                  # envoi vers le buffer
            except Exception as exc:
                logging.exception("Erreur de parsing WS: %s", exc)

# Exemple d’utilisation
if __name__ == "__main__":
    q = asyncio.Queue(maxsize=10_000)
    asyncio.run(binance_trade_stream("BTCUSDT", q))
```

**Points clés**  

| Aspect | Implémentation | Pourquoi |
|--------|----------------|----------|
| *Ping/Pong* | `ping_interval=20`, `ping_timeout=10` | Empêche la fermeture silencieuse du serveur |
| *Back‑pressure* | `queue.maxsize` + `await queue.put` | Bloque le producteur si le consommateur est en retard, évite la perte de messages |
| *Rate‑limit* | WebSocket n’est pas limité, mais les requêtes REST (ex. snapshot) le sont. Utiliser `asyncio.Semaphore(5)` pour limiter à 5 req/s. |
| *Reconnexion* | `while True: try… except websockets.ConnectionClosed: await asyncio.sleep(2)` | Circuit breaker minimal, à combiner avec exponential back‑off (voir 2.5) |

---

### 2.4 Connecteur Kraken (REST + WebSocket)  
...
def predict_gb(features: np.ndarray) -> float:
    ...
    output = InferRequestedOutput(name="output")
    resp = triton.infer(model_name="gb", inputs=[input_tensor], outputs=[output])
...
| **VaR (95 %)** | `VaR = z * σ_portfolio * sqrt(h)` où `z=1.645`, `σ_portfolio` la volatilité du portefeuille, `h` le horizon (jours). | ```python\ndef portfolio_var(vol, horizon_days, z=1.645):\n    return z * vol * np.sqrt(horizon_days)\n``` |

### Pièges fréquents
1. **Over‑allocation** : multiplier la Kelly fraction par un facteur > 1 entraîne un **draw‑down** exponentiel.  
2. **Slippage non modélisé** : le back‑test sous‑estime les coûts si le modèle de slippage est trop optimiste (ex. `0.01 %` fixe).  
3. **Risque de corrélation** : appliquer la même `f` à plusieurs actifs sans corrélation peut dépasser le capital disponible.  
...
# Vectorbt gère automatiquement les SL/TP via Portfolio.from_signals
...
| **Helm release** | Déployer via Helm chart versionné (`Chart.yaml` `appVersion: tag`). | `helm upgrade --install inference ./charts/inference --set image.tag=tag --namespace prod` | `helm status inference -n prod` montre *deployed* et *revision* = N. |
| **Canary** | Créer un *Deployment* avec `strategy: RollingUpdate` et `maxSurge: 25%`, `maxUnavailable: 0`. Utiliser *Argo Rollouts* pour le canary. | `rollout.yaml` avec `steps:` → 10 % → 30 % → 100 % | `kubectl argo rollouts get rollout inference -n prod` indique le pourcentage actuel. |
| **Rollback** | En cas de dégradation du SLO, déclencher `kubectl argo rollouts undo`. | Policy `analysis: { successRate: "99.5%" }` | Le rollback se produit automatiquement si le taux d’erreur HTTP 5xx > 0.5 % pendant 2 min. |
...
@app.post("/predict")
async def predict(payload: TradePayload):
    start = time.time()
    try:
        # ... inference logic ...
        result = model.run(payload)
        REQ_COUNT.labels(model="price_forecast", status="success").inc()
        return {"prediction": result}
    except Exception as exc:
        REQ_COUNT.labels(model="price_forecast", status="error").inc()
        raise HTTPException(status_code=500, detail=str(exc))
    finally:
        REQ_LATENCY.labels(model="price_forecast").observe(time.time() - start)
```

*Vérifiable* : `curl http://localhost:8080/metrics | grep inference_requests_total` renvoie deux lignes (`status="success"` et `status="error"`).
  ...
  - name: inference_slo
    rules:
      - alert: InferenceErrorRateHigh
        expr: sum by (model) (rate(inference_requests_total{status="error