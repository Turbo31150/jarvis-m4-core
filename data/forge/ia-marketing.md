...
**Objectif d’apprentissage** : Implémenter, évaluer et déployer un modèle de prédiction du CPC (coût par clic) avec une précision supérieure à 85 % (R²).  
...
**Objectif d’apprentissage** : Concevoir un système de clustering qui identifie au moins trois segments d’utilisateurs distincts avec un indice de silhouette > 0,5.  
...
**Objectif d’apprentissage** : Mettre en place une boucle d’optimisation qui ajuste les enchères CPC toutes les 15 minutes en fonction du ROAS (Return on Ad Spend) cible.  
...
**Objectif d’apprentissage** : Produire un rapport d’explicabilité conforme au RGPD qui détaille les facteurs influençant les décisions de ciblage pour 100 % des prédictions.  
...
#### 1.3.1 Règle de base  
- **Train** : 60‑70 %  
- **Validation** : 15‑20 % (utilisé pour le réglage d’hyper‑paramètres)  
- **Test** : 15‑20 % (évaluation finale, jamais vu pendant l’entraînement)
...
| **Indice de silhouette** | \(s(i)=\frac{b(i)-a(i)}{\max\{a(i),b(i)\}}\) où *a(i)* est la distance moyenne intra‑cluster et *b(i)* la distance moyenne à l’autre cluster le plus proche. Valeur dans \([-1,1]\). | Mesure de la cohérence d’un clustering ; seuil pratique > 0,5 indique des clusters bien séparés. |
...
| **Agglomerative (Ward)** | Fusion hiérarchique, distance Ward minimise la variance intra‑cluster. | \(O(n^2·p)\). | Produit un dendrogramme exploitable, mais coûteux en mémoire pour > 10 k points. |
...
# ----------------------------------------------------------------------
# 4. Recherche du k optimal (2 ≤ k ≤ 10) via silhouette
# ----------------------------------------------------------------------
