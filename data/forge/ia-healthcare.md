...
MODEL_DRIFT = Gauge(
    ...
    "Score de drift du modèle (0 = stable, 1 = drift critique)",
    ["model_version"]
)
...
def handle_triage(payload: dict) -> dict:
    ...
    time.sleep(random.uniform(0.05, 0.2))   # latence variable
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            pass
        # Exemple de mise à jour du drift toutes les minutes
        update_drift_score("llama2-7b-v1", random.uniform(0, 0.3))
        time.sleep(1)
```
...
|--------------|-------|--------------|--------------------------|
| **Adversarial** | TextAttack (Python) | taux d’erreur sur inputs perturbés | > 5 % |
| **Out‑of‑Distribution (OOD)** | Scikit‑learn – IsolationForest | score d’anomalie moyen | > 0.7 |
| **Drift de données** | Evidently AI | `data_drift` (p‑value) | p < 0.05 |
| **Drift de modèle** | Evidently AI – `model_performance` | différence de ROC‑AUC | Δ >
