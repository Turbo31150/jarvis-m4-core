...
**Objectif mesurable** : Concevoir et déployer un pipeline complet (scraping, enrichissement, actions) fonctionnant sur un compte LinkedIn professionnel en moins de 2 h.  
**Notions couvertes**  
...
**Objectif mesurable** : Implémenter un scraper fiable qui récupère au moins 95 % des champs clés (nom, poste, entreprise, compétences) d’un profil public sans déclencher de blocage.  
**Notions couvertes**  
...
**Objectif mesurable** : Produire, à partir d’un profil brut, un résumé de 150 à 200 mots et une liste de 5 arguments de prise de contact pertinents, avec un taux de pertinence ≥ 80 % évalué par un jeu de validation interne.  
**Notions couvertes**  
...
**Objectif mesurable** : Déployer un bot qui envoie automatiquement 30 invites personnalisées par jour, avec un taux d’acceptation cible de 15 % sur un segment de prospects défini.  
**Notions couvertes**  
...
| 3. | Utiliser **few‑shot** (exemples) | Ajouter 1‑2 exemples de sortie dans le même appel afin de contraindre le format. |
| 4. | Limiter le nombre de tokens | Le modèle GPT‑4‑turbo (2024‑08) accepte 128 k tokens ; garder le prompt < 2 k tokens pour laisser de la marge aux réponses. |
...
|-----------|------------------------|--------------------------|
| **Batching** | Regrouper 10 profils dans un même appel en utilisant le paramètre `messages` avec plusieurs `user` messages. | Réduction de 30 % du coût de tokenisation (OpenAI facture 0,0005 $ / 1 k tokens pour gpt‑4o-mini). |
| **Caching** | Mémoire locale (Redis) : clé = hash SHA‑256 du JSON du profil, valeur = réponse complète. | Évite les appels répétés ; pour 1 000 profils récurrents, économie estimée ≈ 0,20 $. |
| **Limite de débit** | `time.sleep(random.uniform(0.8, 1.2))` entre les appels pour rester sous 60 req/min (limite officielle de l’API). | Conformité aux SLA d’OpenAI, évite les 429 Too Many Requests. |
...
Un seuil de **80 %** de pertinence est atteint lorsque :  
- ROUGE‑L ≥ 0,75, **et**  
- Similarité sémantique ≥ 0,80, **et**  
