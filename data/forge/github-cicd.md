...
- Le cache évite de retélécharger les gros blobs (> 1 Go) à chaque run.  

**Piège** : la taille maximale d’un cache est 5 GiB. Un cache qui dépasse cette limite échoue silencieusement et le job continue sans cache. Surveillez la taille avec `du -sh ${{ env.HF_HOME }}/.cache` dans un step de debug.  
...
**Piège** : les artefacts sont conservés 90 jours par défaut. Un job qui dépend d’un artefact produit il y a plus de 90 jours échouera. Planifiez le nettoyage ou la régénération régulière.  
...
* Faire échouer le workflow dès qu’une régression (test qui retourne un code ≠ 0 ou une couverture < 80 %) est détectée.  
...
| **coverage‑fail‑under** | Option `--cov-fail-under=80` qui fait échouer `pytest` si la couverture globale < 80 %. |
...
jobs:
  test:
    # 1️⃣ Stratégie de matrice : 4 versions Python, GPU optionnel
    strategy:
      matrix:
