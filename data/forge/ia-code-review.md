...
**Objectif mesurable :** Implémenter un pipeline qui, à partir d’un dépôt Git, génère des tokens, AST et métriques de complexité pour chaque fichier, avec un taux de couverture ≥ 95 % sur un jeu de 100 repos.  
**Notions couvertes**  
...
**Objectif mesurable :** Produire, à partir d’un jeu d’entraînement de 5 000 revues humaines, un modèle capable de générer des commentaires pertinents avec un score BLEU ≥ 0,30 et un taux de faux positifs < 10 %.  
**Notions couvertes**  
- Construction de prompts chainés (system, user, assistant) pour guider la génération.  
- Techniques de fine‑tuning LoRA et QLoRA sur modèles de 7 B paramètres.  
...
**Objectif mesurable :** Développer un format de sortie (Markdown + JSON) qui classe les commentaires par sévérité et propose des correctifs automatiques, avec un taux d’acceptation utilisateur ≥ 80 % lors d’un test A/B.  
**Notions couvertes**  
...
- Le nombre de paramètres entraînés est `2 * r * d` (d = dimension du poids), soit < 0,1 % du modèle complet.  
...
7. Le job `cleanup` garantit la libération de l’espace disque, indispensable sur les runners partagés (quota ≈ 14 GB).  
