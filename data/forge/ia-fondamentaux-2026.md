...
> Référence `ia-fondamentaux-2026` · 39 €

## Plan

## Module 1 – Architectures de modèles de fondation en 2026  
**Objectif mesurable** : expliquer le fonctionnement d’un transformeur à grande échelle (≥ 10 M paramètres) et identifier les composants clés d’une architecture mixture‑of‑experts (MoE) utilisée dans les modèles de 2025‑2026.  
**Notions couvertes**  
- Transformer standard : couche d’attention multi‑têtes, normalisation LayerNorm, feed‑forward Gated Linear Units (GLU)  
- Sparse attention (Longformer, BigBird, FlashAttention 2) : schéma de connexion, complexité O(n·log n) ou O(n)  
- Mixture‑of‑Experts (MoE) : routage par top‑k, capacité de paramétrisation, équilibrage de charge  
- Modèles hybrides vision‑langage (ViT‑GPT, Flamingo‑2) : fusion d’embeddings, alignement cross‑modal  
...
**Objectif mesurable** : concevoir un pipeline de collecte, filtrage et augmentation de données capable de produire un jeu d’entraînement de > 1 TB avec un taux de bruit < 0.5 % détecté par des métriques de qualité automatisées.  
**Notions couvertes**  
- Sources de données massives (WebText‑2023, LAION‑5B, Common Crawl 2025) et licences d’utilisation  
...
**Objectif mesurable** : configurer un entraînement distribué sur ≥ 8 GPU A100 80 GB (ou équivalent) en utilisant DeepSpeed/ZeRO‑3 et atteindre une perte de validation ≤ 2.1 % sur le benchmark C4 après 200 k pas de gradient.  
**Notions couvertes**  
...
|----------|----------|----------------------------|
| **Embedding** | Convertit chaque token en vecteur dense | `X ∈ ℝ^{L×d}` où `L` = longueur de séquence, `d` = dimension du modèle (ex. 1024) |
...
Total ≈ `12 d²` ≈ **12 M** paramètres pour `d=1024`.  
...
| **BigBird** | Random + sliding + global | `O(L·(w + r + L_g))` (`r` = nombre de connexions aléatoires, typiquement 3) |
| **FlashAttention 2** | Dense mais implémentation **O(L²)** en temps, **O(L)** en mémoire grâce à kernel fusion | Pas de réduction de complexité algorithmique, mais permet `L≈64k` sur A100 80 GB |
...
|--------|-----------------------------|----------------|--------------------|
| **WebText‑2023** | 45 GB (texte brut) | Articles, forums, blogs | **CC‑BY‑SA‑4.0** (extrait de Reddit) |
| **LAION‑5B** | 5 B d’images ≈ 650 TB (JPEG, 256 px) | Image‑texte (captions) | **CC‑BY‑4.0** (images) + **CC‑0** (captions) |
| **Common Crawl 2025** | 350 TB (raw WARC) | Pages web, code, PDF | **CC‑0** (public domain) – attention aux sous‑licences : certains sites imposent **robots.txt** ou **Terms of Service** restrictifs. |
| **AudioSet‑v2** | 2 TB (audio + tags) | Clips audio 10 s | **CC‑BY‑4.0** (YouTube) – filtrage obligatoire des contenus protégés par droits d’auteur. |
| **Multilingual Wikipedia Dumps** | 30 GB (texte) | Articles encyclopédiques | **CC‑BY‑SA‑3.0** (Wikimedia) |
...
def filter_toxic(dataset_path: str, out_path: str):
    with open(dataset_path, "r", encoding="utf-8") as fin, \
        ...
        for line in tqdm(fin):
            ...
            time.sleep(0.05)   # respect du quota public (20 req/s)
```
...
def is_duplicate(doc_id: str, text: str) -> bool:
    ...
    return any(nb != doc_id for nb in neighbours)

# Exemple d’usage sur un flux gzip
def dedup_stream(in_gz: str, out_gz: str):
    with gzip.open(in_gz, "rt", encoding="utf-8") as fin, \
         gzip.open(out_gz, "wt", encoding="utf-8") as fout:
        for line in fin:
            obj = json.loads(line)
            doc_id = obj["id"]
            txt = obj["text"]
            if not is_duplicate(doc_id, txt):
                add_to_lsh(doc_id, txt)
                fout.write(json.dumps(obj) + "\n")
```

**Points critiques**  

* **Mémoire** : chaque MinHash occupe ~
...
|------|--------|----------|----------------------|--------------------------|
| **Data‑parallelism** | GPU‑level | Chaque GPU possède une copie complète du modèle ; le mini‑batch est découpé en sous‑batches. | All‑reduce des gradients (O(P·log G), P = taille du paramètre, G = nb de GPU). | `torch.nn.parallel.DistributedDataParallel` (DDP). |
| **Tensor‑parallelism** | Op‑level | Un même tenseur (ex. matrice de QKV) est découpé sur plusieurs GPU ; chaque GPU calcule une partie de l’opération. | All‑gather / reduce‑scatter (O(P/G)). | Megatron‑LM `ColumnParallelLinear`, `RowParallelLinear`. |
| **Pipeline‑parallelism** | Layer‑level | Le réseau est partitionné en *stages* ; chaque GPU exécute un sous‑ensemble de couches, les activations sont transmises en flux. | Latence de pipeline + all‑reduce des gradients de chaque stage. | `torch.distributed.pipeline.sync.Pipe`. |

**Combinaison typique (8 GPU A100 80 GB)**  

```
# 8 GPU → 2 nodes × 4 GPU
# 2‑way tensor‑parallelism (split QKV matrices)
# 2‑way pipeline‑parallelism (4 layers / stage)
# Data‑parallelism sur les 2 groupes restants
```

### 3.1.1 Calcul de la capacité mémoire  

Modèle : Transformer ≈ 10 M paramètres → 40 Mo en FP32, 20 Mo en BF16.  
Activations : L = 24 couches, B = 64, S = 2048, H = 1024 →  
`activations ≈ B·S·H·L·2 bytes ≈ 3 GiB` en BF16.  

Avec ZeRO‑3, **tous les paramètres, gradients et états d’optimiseur sont sharded** → chaque GPU ne garde que `1/ (tensor‑par × data‑par)` du total.  
Exemple : tensor‑par = 2, data‑par = 2 → chaque GPU stocke ≈ 5 MiB de paramètres + 5 MiB de gradients + 5 MiB d’états = **≈ 15 MiB**.  
Le facteur limitant devient la taille des activations + buffers de communication (≈ 3 GiB).  

## 3.2 Optimiseurs de dernière génération  

| Optimiseur | Mémoire supplémentaire | Particularité | Usage recommandé |
|------------|------------------------|---------------|-----------------|
| **AdamW + LAMB** | 2× paramètres (moments) | Scales le LR par couche, stable pour batch > 4 k | Fine‑tuning de modèles très larges. |
| **Adafactor** | 1× paramètres (facteur) | Pas de moments explicites, bon pour très grands modèles (≥ 1 B). | Pre‑training de modèles > 100 M param. |
| **ZeRO‑3 (DeepSpeed)** | 0 (ou très faible) | Sharding complet, compression 8‑bit optional. | Tout entraînement distribué > 4 GPU. |
...
- `offload_param`/`offload_optimizer` permettent de dépasser la capacité GPU 80 GB en stockant les paramètres et états sur le CPU + NVMe, tout en gardant les activations en GPU.  
- `stage3_max_live_parameters` doit être supérieur au nombre de paramètres actifs simultanément (≈ 10 M × 4 bytes = 40 MiB).  
...
| Technique | Niveau de précision | Gains typiques (sur A100 80 GB) | Impact sur la qualité (Δ BLEU / Δ ROUGE) | Contraintes |
|-----------|--------------------|---------------------------------|------------------------------------------|-------------|
| **Post‑training INT8** (static) | INT8 | × 2 – 3 × débit, ‑ ≈ 30 % de RAM | ‑ 0.3 % BLEU (modèle 7 B) | Nécessite calibration sur ≥ 100 k tokens représentatifs. |
| **Post‑training FP4** (4‑bit) | FP4 (E4M3) | × 4 – 5 × débit, ‑ ≈ 70 % de RAM | ‑ 0.6 % BLEU (modèle 7 B) | Sensible aux valeurs extrêmes; appliquer *weight‑only* quantisation. |
| **Quant‑aware training (QAT)** | INT8 | × 2 – 3 × débit, ‑ ≈ 30 % de RAM | < 0.1 % BLEU (modèle 7 B) | Nécessite ré‑entraînement (≈ 10 % des steps). |
| **Compilation via `torch.compile`** | FP16/BF16 | × 1.5 – 2 × débit (CPU) ou × 1.2 – 1.5 × débit (GPU) | Aucun | Le graph doit être *static* (pas de `torch.jit.script` + `torch.compile` simultané). |
...
# 2️⃣ Calibration dataset (10 k sentences, déjà tokenisées)
calib_dataset = torch.load("calib_dataset.pt")  # Tensor[10000, seq_len]
