...
> Référence `monitoring-ia` · 69 €

## Plan

## Module 1 : Architecture de monitoring pour modèles IA  
**Objectif d’apprentissage** : Concevoir et déployer une architecture de monitoring capable de collecter, stocker et visualiser les métriques d’inférence d’un modèle en production avec un taux de disponibilité ≥ 99 %.  
...
- Latence moyenne, p95/p99, taux d’erreur HTTP ; mesure avec client‑side instrumentation.  
...
**Objectif d’apprentissage** : Dimensionner le système de monitoring pour supporter ≥ 10 000 RPS tout en maintenant un overhead CPU < 5 % sur les services monitorés.  
...
|----------|---------|------------------|------------------------|
| **Latence moyenne** | `mean(latency_ms)` | Temps entre réception de la requête HTTP et renvoi du corps de réponse | 1 s |
| **p95 / p99 latence** | `percentile(latency_ms, 0.95)` | Même source que ci‑dessus | 1 s |
| **Débit (RPS)** | `count(requests) / interval` | Compteur d’appels HTTP | 10 s |
| **Taux d’erreur HTTP** | `sum(errors) / sum(requests)` | `errors = status >= 500` | 10 s |
...
def infer(payload):
    ...
    start = time.time()
    # Simuler temps de calcul (10‑200 ms) et erreurs aléatoires (1 %)
...
if __name__ == "__main__":
    # Expose /metrics sur le port 8000
...
Les métriques de classification sont agrégées sur un **fenêtre glissante** (ex. 5 min) afin d’éviter le stockage complet du jeu de validation.  
...
# Structure de stockage : deque de taille fixe (ex. 3000 échantillons)
WINDOW_SIZE = 3000
...
| **Expression** | Requête PromQL qui renvoie une série booléenne (1 = alerte, 0 = OK). | `sum(rate(http_requests_total{job="model-serving",code=~"5.."}[5m])) / sum(rate(http_requests_total{job="model-serving"}[5m])) > 0.02` |
| **For** | Durée pendant laquelle la condition doit rester vraie avant de passer en état *firing*. Empêche les « flipping ». | `for: 2m` |
| **Labels** | Metadonnées utilisées par Alertmanager (severity, service, team). | `labels: severity: critical` |
| **Annotations** | Texte affiché dans les notifications (summary, description). | `annotations: summary: "Taux d’erreur HTTP 5xx > 2 %"` |
    ...
    rules:
      - alert: HighHttp5xxRate
        expr: |
          ...
          /
          sum(rate(http_requests_total{job="model-serving"}[5m])) > 0.02
        for: 2m
        labels:
          severity: critical
          service: model-serving
        annotations:
          summary: "Taux d’erreur HTTP 5xx > 2 % sur le service de modèle"
          description: |
            Le service {{ $labels.service }} génère plus de 2 % d’erreurs 5xx sur les
            5 dernières minutes (actuel : {{ $value | printf \"%.2f\" }}).
```
...
|------------|---------------|--------|
| Alertes qui s’enclenchent puis disparaissent en quelques secondes | `for` trop court ou métrique bruitée | Augmenter `for` (ex. 2 min) ou appliquer un `rate(...[5m])` plus large |
...
| Latence accrue > 5 ms | TLS handshake coûteux sur chaque scrape. | Activer le **keep‑alive** HTTP (`http_config: { idle_conn_timeout: 30s }`) ou passer à un **push‑gateway** avec authentification mTLS. |
...
| Composant | Rôle | Paramètre clé | Valeur typique pour ≥ 10 000 RPS |
|-----------|------|----------------|-----------------------------------|
| **Prometheus** (instance “edge”) | Scrape les métriques locales (Node Exporter, OpenTelemetry Collector) | `scrape_interval` | 15 s |
| **Prometheus Federation** | Agrège les séries de plusieurs “edge” vers un “central” | `honor_labels: true` + `match[]` | 1 k séries/instance |
| **Cortex / Thanos** | Stockage à long terme et mise à l’échelle horizontale | `replication_factor` | 3 réplicas |
| **Remote Write** | Envoi asynchrone vers Cortex/Thanos | `queue_config.max_shards` | 8 shards |
| **Prometheus Operator** (K8s) | Gestion du lifecycle | `replicas` (statefulset) | 3 pods (HA) |
...
| Algorithme | Implémentation dans Prometheus | Ratio de compression moyen | Overhead CPU |
|-----------|-------------------------------|---------------------------|--------------|
| **Gorilla** (Delta‑of‑Delta) | natif, utilisé pour les blocs TSDB | 3–5 × | < 1 % |
| **Snappy** (block compression) | appliqué sur chaque bloc de 2 h | 2–3 × | < 2 % |
| **ZSTD (Cortex/Thanos)** | option `-storage.tsdb.compaction.max-block-duration` | 5–7 × | 3–4 % |
...
| Agent | Métrique de scaling | HPA target | Min/Max pods |
|------|----------------------|-----------|--------------|
| **Prometheus Node Exporter** | `cpu_utilization_percentage` | 70 % | 2 / 20 |
| **Telegraf** (collecte logs) | `memory_usage_bytes` | 75 % | 1 / 15 |
| **OpenTelemetry Collector** | `custom_metric:otc_queue_length` | 1000 samples | 2 / 30 |
...
*Le collector expose `otc_queue_length` via le exporter `/metrics`. Le HPA augmente le nombre de pods dès que la file d’attente dépasse 1000 samples, évitant la perte de points de données.*

### 5.4 Benchmarking de charge  

| Outil | Scénario | Commande de base |
|------|----------|-----------------|
| **k6** | 10 000 RPS sur `/metrics` (scrape) | `k6 run --vus 200 --duration 5m script.js` |
| **Locust** | 10 000 RPS sur endpoint d’inférence + `/metrics` | `locust -f locustfile.py --users 5000 --spawn-rate 1000` |
