...
**Notions couvertes**  
- Prérequis matériels : CPU ≥ 8 cœurs, RAM ≥ 32 Go, GPU NVIDIA RTX 3080 ou supérieur, driver ≥ 525 et CUDA 11.8.  
...
- Gestion du débit (`rate limiting` 60 req/min) et des retries exponentiels (`Retry-After`).  
...
valid_data: data/valid.jsonl    # optionnel, sinon split aléatoire 10 %
output_dir: fine_tuned/        # répertoire où seront écrits les checkpoints
epochs: 3
batch_size: 4                  # dépend de la VRAM disponible (≈ 8 GB/GPU)
learning_rate: 2e-5
max_grad_norm: 1.0
seed: 42
```
...
| **Reverse‑proxy** | TLS termination, rate‑limit, métriques | `nginx` (ou `traefik`) | Certificat valide, `proxy_read_timeout` ≥ 30 s |
...
| **Metrics exporter** | Exporter les stats Prometheus | `prometheus_client` intégré à `mistral` (`/metrics`) | Scraping interval ≤ 15 s |
...
### 4.3 Configuration `mistral.yaml` adaptée à la prod  

```yaml
model_path: "/app/model/mistral-2.3.0.gguf"
log_dir: "/app/logs"
max_threads: 12                # garde 2 cœurs libres pour le OS
cache_size: 8GB                # 1/4 de la RAM disponible (32 GB)
request_timeout: 25            # seconds, > client timeout
max_concurrent_requests: 200  # limite interne, gérée par le reverse‑proxy
```

> **Vérification** : `docker exec -it <container> mistral health` doit renvoyer `status:
...
- `logging.level` : utilisez `DEBUG` uniquement en phase de développement, il génère ~30 % de trafic supplémentaire sur le disque.
...
- `--workers` crée un pool de processus isolés, chacun charge le modèle en mémoire. La RAM requise ≈ `model_size` × `workers`.  
- `LimitNOFILE` doit être supérieur au nombre de requêtes simultanées × 2 (socket + fichier log).
...
Alerte si la latence dépasse 200 ms pendant 3 intervalles consécutifs.
"""
import time
import requests
...
LATENCY_BUCKET = 0.2          # 200 ms
CONSECUTIVE_THRESHOLD = 3    # alertes consécutives requises
INTERVAL = 30                 # secondes entre chaque scrape
