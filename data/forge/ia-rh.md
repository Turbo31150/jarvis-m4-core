...
**Objectif mesurable** : Implémenter un pipeline automatisé en Python pour nettoyer, normaliser et enrichir les CV, profils LinkedIn et bases internes, avec un taux d’erreur de parsing < 2 %.  
**Notions couvertes**  
...
> **Note technique** : En RH, les jeux de données sont souvent déséquilibrés (ex. : 5 % de candidatures retenues). Le choix de la tâche influe directement sur les métriques à surveiller.
...
> **Précision de calcul** : Pour le **Recall** en RH, on considère généralement le « candidat réellement qualifié » comme positif, même si le label a été attribué a posteriori (ex. : embauché après 6 mois).  
...
| PDF (scanné) | `pdf2image` + `pytesseract` | Qualité d’image ≥ 300 dpi recommandée, sinon taux d’erreur > 5 %. |
...
2. **Extraction d’entités nommées (NER)**  
   - Modèle de base : `fr_core_news_md` de spaCy (≈ 20 M de paramètres).  
   - Fine‑tuning sur un corpus de 5 000 CV annotés : `spacy train` avec `ner` et `entity_ruler`.  
...
3. **Normalisation des compétences**  
   ...
   - Algorithme de mapping : recherche du **k‑nearest neighbour** (k=3) dans l’espace ESCO, puis vote majoritaire sur le libellé.  

4. **Gestion des valeurs manquantes**  
   - `experience_years` : imputation par la médiane par groupe de fonction (`job_title`).  
   - `skills` : si vide, récupérer les mots‑clés du texte brut via TF‑IDF (seuil 0,2).  
...
## 2.3 Exemple complet : parsing d’un CV PDF → DataFrame  

```python
# -*- coding: utf-8 -*-
"""
Pipeline minimal : PDF → texte → NER → mapping compétences → DataFrame
"""
...
# ----------------------------------------------------------------------
# 1. Chargement des modèles (une fois, à l'initialisation du service)
# ----------------------------------------------------------------------
nlp = spacy.load("fr_core_news_md")               # NER de base
skill_extractor = SentenceTransformer(
    "sentence-transformers/all-MiniLM-L6-v2"
)                                                # embeddings compétences
# ESCO embeddings pré‑calculés (exemple : 15 000 libellés)
ESCO_PATH = pathlib.Path("data/esco_embeddings.pt")
ESCO_LABELS = pathlib.Path("data/esco_labels.json")
esco_emb = torch.load(ESCO_PATH)                 # Tensor[15000,384]
with open(ESCO_LABELS, "r", encoding="utf-8") as f:
    esco_labels = json.load(f)                   # list[str]
...
def extract_text_from_pdf(path: pathlib.Path) -> str:
    """Retourne le texte brut d'un PDF texte. Lève IOError si vide."""
    txt = pdfminer.extract_text(str(path))
    if not txt.strip():
        raise IOError("PDF vide ou uniquement image")
    return clean_text(txt)

def ner_skills(doc) -> List[str]:
    """Retourne la liste
...
| Contraintes | - Recall ≥ 0,80 <br> - Taux de faux‑positifs (FPR) <br> < 0,15 <br> - Modèle exploitable en production (explainability) |
...
Il s’agit d’un **problème de classification binaire** avec un déséquilibre typique : le taux de churn (départs) varie entre 5 % et 15 % selon les secteurs. Le modèle doit maximiser la capacité à détecter les vrais départs (high recall) tout en limitant les alertes inutiles (low FPR).
...
sm = SMOTE(sampling_strategy=0.3, random_state=42)   # 30 % de churn après sur‑échantillonnage
...
Pour atteindre le **recall ≥ 0,80** tout en contrôlant le **FPR**, on privilégie un **XGBoost** avec réglage du paramètre `scale_pos_weight` et du seuil de décision.
