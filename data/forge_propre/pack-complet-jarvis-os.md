# Pack Intégral — Formations

> Référence `pack-complet-jarvis-os`

## Plan

## Module 1 – Fondamentaux de l’IA générative avec Jarvis‑OS  
**Objectif mesurable** : Être capable de créer, entraîner et évaluer un modèle de texte génératif basique sous Jarvis‑OS dans un délai raisonnable.  
...
**Objectif mesurable** : Concevoir un chatbot capable de répondre à des requêtes métier avec une précision élevée sur un jeu de test représentatif.  
...
**Objectif mesurable** : Générer des images à partir de descriptions textuelles et extraire du texte d’images avec un taux de réussite élevé sur un jeu de validation.  
...
**Objectif mesurable** : Réduire le temps d’inférence d’un modèle sur un serveur GPU tout en conservant la précision.  
...
> **Note** : GPT‑2‑small possède plusieurs blocs d’auto‑attention, une dimension d’embedding importante et plusieurs millions de paramètres.  
...
| CUDA Toolkit | 12.1 |
| Python | 3.11 |
| GPU | au moins 8 GB de VRAM (ex. RTX 3090, A100) |
...
steps = 30                     # DPM‑Solver++ converge rapidement
...
|-----------|---------|-------|--------|
| OOM (out‑of‑memory) dès le premier batch | Crash du processus Docker | Batch size trop important pour la VRAM disponible | Fixer `batch_size=1` ou activer `torch.cuda.empty_cache()` entre les appels |
| Image floue ou bruitée malgré un nombre de steps insuffisant | Scheduler mal configuré | Utilisation du *Euler‑a* scheduler avec trop peu de steps | Passer à `scheduler="dpm_solver++"` et augmenter le nombre de steps |
| Texte du prompt ignoré | `guidance_scale` trop bas | Valeur insuffisante ne force pas le conditionnement | Utiliser un `guidance_scale` plus élevé pour la plupart des prompts |
...
- **Principe** : Ajoute deux matrices `A` et `B` à chaque poids `W` du UNet : `W' = W + α·A·B`.  
- **Avantages** : Mémoire GPU réduite, entraînement rapide sur un A100 pour un petit jeu de données.  
...
|-----------|------------------------|--------------|----------------|
| **Quantisation post‑training (PTQ)** | `jarvis.quantizer` | INT8, FP16 | Réduction notable de la latence, perte de précision très faible si calibration correcte |
| **Quantisation aware training (QAT)** | `jarvis.quantizer.qat` | INT8 | Amélioration de la précision par rapport au PTQ, nécessite quelques époques de ré‑entraînement |
| **Élagage (pruning)** | `jarvis.pruner` | FP32/FP16 | Diminution importante du nombre de paramètres, gain de bande passante mémoire, perte de précision dépend du taux d’élagage |
...
# 2️⃣ Définir le calibrateur : on utilise plusieurs batches d’un jeu de calibration
...
*Points de vigilance*  
- **Calibration** : le jeu de calibration doit couvrir la même distribution que la charge de production (longueur de séquence, vocabulaire). Un jeu trop petit entraîne un **scale** inadapté → sous‑ou sur‑estimation des valeurs, perte de précision notable.  
- **Per‑channel vs per‑tensor** : le premier conserve plus de précision mais augmente la taille du tableau de `scale`/`zero_point`. Certains back‑ends ne le supportent pas.  
...
pruner = GlobalMagnitudePruner(
    model=model,
    # target_sparsity=...,  # proportion des poids à mettre à zéro, à définir selon les besoins
    prune_type="unstructured",    # pruning fine‑grained
)

pruned_model = pruner.prune()
# Optionnel : fine‑tuning de quelques epochs pour récupérer la précision
pruned_model.train()
...
save_model(pruned_model, "gpt2_pruned.pt")
print("Modèle élagué sauvegardé.")
```

*Pièges*  
- **Élagage non‑structuré** : les GPU modernes ne profitent que partiellement du sparsity non‑structuré ; la latence ne diminue pas proportionnellement au taux d’élagage.  
- **Fine‑tuning insuffisant** : un nombre limité d’epochs peut ne pas suffire pour récupérer la perte de précision. En pratique, plusieurs epochs avec un LR réduit sont recommandés.  
