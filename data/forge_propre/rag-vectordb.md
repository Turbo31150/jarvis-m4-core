...
> Référence `rag-vectordb`

## Plan

## Module 1 : Principes fondamentaux du Retrieval‑Augmented Generation (RAG)  
**Objectif mesurable** : L’apprenant pourra expliquer le flux de données d’un système RAG et implémenter un pipeline simple (requête → recherche → génération) dans un délai raisonnable.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant sera capable de créer, peupler et interroger une base vectorielle (ex. FAISS, Annoy, Milvus) avec un taux de rappel élevé sur un jeu de test de taille substantielle.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant pourra fine‑tuner un modèle de passage retriever (ex. DPR, Contriever) sur un jeu de données propriétaire et atteindre un MRR élevé.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant implémentera un service RESTful qui combine recherche vectorielle et génération de texte, avec une latence faible pour la phase de récupération.  
**Notions couvertes**  
...
* `tokenizer(..., truncation=True)` évite les dépassements de longueur maximale (une limite typique pour la plupart des BERT).  
...
|-----------|------------|-----------------|------------------------|-------------------------------|
| IVF‑Flat | Inverted File, clusters k‑means | L2 ou inner‑product | O(1) (ajout à un posting list) | O(log n) + O(k · C) |
| IVF‑PQ | IVF + Product Quantization | L2 | O(1) | O(log n) + O(k) |
| HNSW | Hierarchical Navigable Small World graph | L2 ou inner‑product | O(log n) | O(log n) |
| Flat | Brute‑force | L2 ou inner‑product | O(1) | O(n) (déconseillé pour de très grands ensembles) |

**Choix pratique**  

* **Petit jeu de taille modeste** → `IndexFlatIP` (exact, aucune phase d’entraînement).  
* **Jeu de taille moyenne à grande** → `IndexIVFFlat` avec un nombre de listes proportionnel à la racine carrée du nombre de vecteurs.  
* **Très grand jeu avec contrainte de latence stricte** → `IndexHNSW` avec un paramètre de connectivité adapté.  
...
* **Centres non‑zero** : si les vecteurs sont L2‑normalisés, le centre est déjà proche de zéro ; centrer à nouveau introduit un biais.  
* **Perte de précision** : la plupart des modèles de passage conservent la majeure partie de la variance dans les premières composantes ; réduire fortement la dimension peut diminuer le rappel.  
...
| 3.1.4 | Splitter **train / dev** (exemple de répartition). | Vérifier que les IDs de passages ne se chevauchent pas entre les deux splits. |
...
|-----------|------|----------------------|
| **FastAPI** | serveur HTTP asynchrone, expose les endpoints RAG | éviter le `await` bloquant, garder le serveur en mode *uvicorn* avec `--workers` |
...
| **gRPC** (optionnel) | appel interne entre le service de recherche et le service de génération, sérialisation binaire (Protobuf) | le coût de sérialisation peut être notable; mesurer le temps de *marshalling* dans les métriques |
...
| **Mise à jour** | Utiliser une **re‑indexation incrémentale** : ajouter les nouveaux vecteurs avec le même `index_name` puis **merge** (FAISS : `index.merge_from`). | Le taux de rappel après le merge doit rester très élevé sur le jeu de validation. |
...
#### Piège concret  
> **Ne pas ré‑indexer les embeddings après un changement de modèle** conduit à un **drift** : les vecteurs sont dans un espace différent de celui du modèle de requête, ce qui fait chuter les performances de manière notable (mesuré sur un benchmark de recherche).  
...
|-----------|-------------------------------|------------------------|------------------------|
| **IVF‑Flat** (FAISS) | `O(log n)` (coarse quantizer) | 4 × d (float32) | Jeux de données de taille modérée, besoin de haute précision. |
| **IVF‑PQ** | `O(log n)` + décodage PQ | 1 × d/8 (8 bits/centroïde) | Jeux très volumineux, contrainte mémoire stricte. |
| **HNSW** | `O(log n)` (graph navigation) | 2 × d (float32) | Recherche ultra‑rapide, tolérance à un léger rappel réduit. |
| **IVF‑HNSW** (FAISS) | `O(log n)` + `O(log n)` | 2 × d (float32) | Compromis entre précision HNSW et filtrage IVF pour des corpus très grands. |

**Règle d’or** : mesurer le rappel et la latence sur un sous‑ensemble représentatif de requêtes avant de déployer en production.  

#### 5.2.2. Quantisation et distillation  

- **Quantisation 8‑bits** (FAISS `IndexIVFPQ` + `faiss.IndexScalarQuantizer`) réduit la mémoire de façon significative, mais le rappel diminue légèrement selon la distribution des vecteurs.  
- **Distillation de modèle d’embedding** (ex. `MiniLM‑L6‑v2` à la place de `MPNet`) diminue la dimension du vecteur, ce qui accélère le calcul de la similarité tout en conservant un bon niveau de performance sur le même jeu.  

**Piège** : appliquer la quantisation **sans recalibrer** les paramètres du `nlist` (IVF) entraîne un déséquilibre de clusters et augmente le nombre de listes vides ; le rappel en pâtit.  
...
REQ_TOTAL = Counter(
    "rag_requests_total",
    "Nombre total de requêtes RAG traitées"
)
...
INDEX_SIZE = Gauge(
    ...
    "Taille actuelle de l'index vectoriel"
)

def start_metrics_server(port=8000):
    """Démarre le serveur HTTP exposant les métriques Prometheus."""
    start_http_server(port)
    print(f"Serveur de métriques démarré sur le port {port}")

# Exemple d'utilisation dans le code de service
...
Ces métriques permettent de suivre le volume de trafic, la latence des requêtes, la taille de l’index et d’autres indicateurs clés pour déclencher des alertes en cas de dérive ou de saturation.