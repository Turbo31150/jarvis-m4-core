...
**Objectif mesurable** : Concevoir et implémenter un pipeline CI/CD complet (build, test, validation, déploiement) pour un modèle de classification.  
...
**Objectif mesurable** : Déployer le modèle en service RESTful sur un cluster Kubernetes et valider le SLA de latence sur un jeu de requêtes synthétique.  
...
|----------|--------|------------------|-------------|
| **Précision / Recall / F1** | API inference (payload → prédiction) | **Evidently AI** – `Report` ou `Dashboard` | Calculé sur un *sample* de requêtes réelles pour éviter la surcharge. |
...
| **Score de dérive (KS, PSI)** | Calculé sur les features clés | **NannyML** – `DriftCalculator` | Le PSI indique la stabilité ou la dérive selon des seuils définis par l'équipe. |
...
| **Erreur d’inférence (exceptions, time‑outs)** | Middleware d’API | **OpenTelemetry** – traces + **Prometheus** counter | Un taux d’erreur déclenche une alerte lorsqu'il dépasse le seuil défini. |
...
- name: model_drift
  ...
  - alert: ModelDataDrift
    expr: evidently_data_drift_drift_score > <threshold>
    for: <duration>
    labels:
      severity: critical
    annotations:
      ...
      description: |
        Le score de dérive (PSI) a dépassé le seuil défini pendant une période prolongée.
...
| **Labels manquants** | Alertmanager ne trouve pas `model_name` ou `dashboard_url`. | Ajouter les labels dans le job Prometheus (`metric{