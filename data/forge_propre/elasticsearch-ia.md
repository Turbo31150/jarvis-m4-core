...
|--------------|---------------------|------------------------------------------|
| **master‑eligible** | Élection du maître, coordination du cluster, mise à jour du state metadata. | Trois nœuds master‑eligible, aucun nœud dédié n’est obligatoire, mais on recommande plusieurs master‑eligible pour le quorum. |
...
- Dans un cluster avec trois master‑eligible, le quorum correspond à la majorité. Si un nœud master‑eligible tombe, le cluster reste opérationnel tant que les deux restants élisent un nouveau maître.  
...
- La répartition est gérée par le **allocation decider** qui prend en compte la capacité disque via les seuils configurables.  
...
|---------|-----------------|
| RAM | Une quantité de mémoire suffisante par nœud, avec le heap limité à la moitié de la RAM et une taille maximale raisonnable |
| CPU | Un processeur multi‑cœur |
