...
- Topologie des nœuds : nombre de serveurs, CPU, GPU (ex. NVIDIA A100, NVLink).  
- Réseau haute‑bande : InfiniBand HDR vs Ethernet, latence et débit impactant la synchronisation des gradients.  
- Partitionnement du stockage : NVMe RAID local, CephFS partagé, persistance des checkpoints.  
- Pilotes et runtime : NVIDIA Driver, CUDA, cuDNN, NCCL pour la communication collective.  
...
- Stratégies de scaling : HPA basé sur l’utilisation de la mémoire GPU, Cluster Autoscaler avec groupes de nœuds.  
...
- Mixed‑precision training : FP16/TF32, utilisation de `torch.cuda.amp`, impact sur le débit.  
...
**Objectif** – L’apprenant pourra configurer un serveur d’inférence capable de servir des requêtes en temps réel avec un taux de réponse très faible pour des prompts de taille modérée.  
- Serveurs d’inférence : TensorRT‑LLM, vLLM, DeepSpeed‑Inference, comparaison des latences et du débit.  
- Gestion du batching : dynamique selon la charge, agrégation des requêtes pour maximiser l’utilisation des GPU.  
- Cache de résultats : mise en place d’un cache de réponses fréquentes pour réduire la charge de calcul.  
- Monitoring : métriques de latence, taux d’erreur, utilisation GPU via Prometheus, alertes configurées.  
- Optimisations logicielles : utilisation de kernels optimisés, réduction des copies mémoire, réglages du scheduler.
---
...
| Élément | Description | Rôle dans le calcul distribué |
|--------|-------------|------------------------------|
| **Serveur** | CPU Intel Xeon multi‑core | Orchestration des processus, gestion du trafic réseau, hébergement du système d’exploitation. |
| **GPU** | NVIDIA A100 avec mémoire haute capacité | Exécution du forward/backward, stockage du modèle en VRAM, calcul des gradients. |
| **Interconnexion GPU‑GPU** | NVLink de génération récente, bande passante élevée | Partage de tenseurs entre GPUs du même nœud, réduction NCCL intra‑node. |
| **Mémoire système** | DDR4 à grande capacité | Buffers d’entrée, jeu de données, caches d’OS. |
| **Stockage local** | NVMe PCIe de grande capacité, configuré en RAID 0 | Check‑points, modèles temporaires, I/O de données d’entraînement. |
| **Réseau inter‑nœuds** | InfiniBand HDR de dernière génération | Synchronisation des gradients (All‑Reduce) entre nœuds, latence très faible. |
| **Alimentation** | Alimentation redondante de haute puissance | Garantit la stabilité sous charge maximale. |

> **Vérifiable** : La bande passante théorique d’un A100 en FP16 est très élevée (plus de 300 TFLOPS en TF32). La bande passante NVLink de deuxième génération dépasse largement les besoins de communication intra‑node.
...
- \(S_{\text{model}}\) : taille du modèle en octets.  
- \(\text{batch\_size\_global}\) : nombre total d’échantillons traités par itération.  
- \(t_{\text{comm}}\) : temps cible pour l’All‑Reduce.

En appliquant la formule à un modèle de plusieurs gigaoctets et à une taille de lot globale importante, on obtient un débit requis qui dépasse largement la capacité d’un lien InfiniBand HDR isolé. On utilise donc le **ring‑All‑Reduce** NCCL, qui répartit la charge sur tous les liens disponibles, réduisant ainsi la contention. En pratique, le débit agrégé du réseau doit être largement supérieur à la taille du modèle par itération pour éviter tout goulot d’étranglement.
...
|-------------|----------------------|----------------|------------|
| InfiniBand HDR (MLX5) | Bande passante très élevée | Latence ultra‑faible | All‑Reduce, pipeline‑parallel, échange de micro‑batches. |
| Ethernet 25 Gb/s | Bande passante modérée | Latence faible | Gestion du trafic d’administration, accès aux services de stockage partagé. |
| Ethernet 100 Gb/s (option) | Bande passante très élevée | Latence réduite | Scénarios de formation à très grande échelle. |

**Pourquoi InfiniBand HDR ?**  
- Sa latence ultra‑faible minimise le temps d’attente du *ring‑All‑Reduce* NCCL.  
- Sa bande passante largement supérieure à la taille du modèle (en FP16) assure que le temps de communication reste une petite fraction du temps total d’une itération.
...
| Runtime | Avantages observés | Contraintes |
|---------|-------------------|-------------|
| **TensorRT‑LLM** (v8.6) | Latence très faible avec FP16 sur plusieurs GPU A100 | Nécessite conversion ONNX → TRT, support limité de certains ops (e.g. `torch.nn.functional.scaled_dot_product_attention` avant v2.1) |
| **vLLM** (v0.5) | Latence très faible avec FP8/INT8, support de LoRA | Consomme plus de RAM CPU pour le pré‑allocation du KV‑cache |
| **DeepSpeed‑Inference** (v0.12) | Latence très faible avec FP16 + ZeRO‑Inf‑2 | Implémentation plus lourde du partitionnement de modèle, nécessite `deepspeed` ≥ 0.12.0 |
...
3. **Appel du modèle** (exemple avec vLLM)  
   ...
   llm = LLM(model="meta-llama/Meta-Llama-3-8B",
             tensor_parallel_size=8,   # nombre de GPU A100
             dtype="float16",
             max_seq_len=4096,
             gpu_memory_utilization=0.90)   # proportion de la VRAM utilisée
...
| Optimisation | Implémentation | Impact observé |
|--------------|----------------|----------------|
| **Tensor Cores FP16/TF32** | `torch.backends.cuda.matmul.allow_tf32 = True` (TF32) ou `dtype=torch.float16` (FP16) | Gain notable en TFLOPS et réduction de la latence |
| **Fusion de kernels** (NVIDIA **Cutlass** via TensorRT) | `trt_engine = torch_tensorrt.compile(model, inputs=[torch.randn(1, seq_len, hidden_dim).to('cuda')], enabled_precisions={torch.float16})` | Moins d’appels CUDA, latence réduite |
| **KV‑cache en FP8** (vLLM ≥ 0.5) | `llm = LLM(..., dtype="float8")` | Réduction importante de la consommation mémoire et amélioration du débit pour de longues séquences |
| **Batching dynamique** | `max_num_batched_tokens = 4096` dans `SamplingParams` | Meilleure utilisation des GPU, latence stable pour des tailles de batch modérées, charge élevée du GPU |
| **Pinned memory for host‑GPU transfer** | `torch.utils.data.DataLoader(..., pin_memory=True)` | Amélioration du débit d’IO |

### 4.4 Gestion du KV‑cache  

- **Dimension** : `cache_size = max_seq_len × num_layers × hidden_dim × 2 × dtype_size`.  
  La taille dépend des paramètres du modèle (longueur maximale, nombre de couches, dimension cachée et type de précision).
---
