...
> **Règle 1** : chaque métrique doit être *SLA‑compatible* (ex. latence doit rester dans les limites définies).  
...
* **Prometheus** : base temporelle, stockage par défaut, `remote_write` vers Cortex/Thanos si besoin de rétention longue.  
...
def fetch_live_features():
    """
    Simule la récupération des dernières valeurs de chaque feature
    depuis un topic Kafka ou une base de logs. Ici on gén
---
...
|------|-------------|-----------------|---------------------|
| **Blue‑Green** | Deux environnements (blue = production, green = nouvelle version). Le basculement se fait en une seule opération `kubectl apply` sur le service. | Kubernetes Service, Helm, Seldon Core | Temps d’arrêt = , test de santé complet avant bascule. |
| **Canary** | Le trafic est réparti progressivement entre la version courante et la version candidate. Le monitoring décide du passage à l’étape suivante. | Argo Rollouts, Istio VirtualService, Flagger, Seldon Core | Métriques de latence, taux d’erreur, KPI métier. |
...
* **Rollback instantané** : la configuration Istio/Argo permet de revenir à la version stable rapidement.  
...
graph LR
    subgraph K8s Cluster
        ...
        B -->| | C[Deployment v1 (Seldon Model)]
        B -->| | D[Deployment v2 (Seldon Model)]
...
spec:
  ...
  - name: default
    ...
    # traffic:  # géré par Argo, valeur initiale définie dynamiquement
```
...
spec:
  ...
  - name: canary
    ...
    # traffic:  # initialement nul, géré par Argo
```
...
spec:
  ...
  - route:
    - destination:
        ...
        subset: v1
      # weight:  # poids initial, géré dynamiquement
    - destination:
