...
|-----------|--------------------|---------------------------|
| **etcd** | KV‑store fortement consistant. Stocke les objets API (Pods, Nodes, CRDs). | Un quorum avec latence assure que la création d’un `Job` ou `Pod` GPU est répercutée rapidement aux nœuds. Un etcd sous‑dimensionné peut ajouter du retard à chaque planification. |
| **API Server** | Point d’entrée RESTful. Authentifie, valide les requêtes, persiste les données. | Le temps de réponse (latence) + temps de validation (admission controllers) + temps de persistance (etcd) = **latence de soumission**. Pour les entraînements interactifs, viser une latence API réduite. |
...
| **Controller‑manager** | Gère les contrôleurs (ReplicaSet, Job, CronJob, etc.). | Le `job‑controller` crée les Pods d’un Job IA. Un intervalle de re‑conciliation trop long retarde la relance en cas d’échec de Pod. On peut réduire le paramètre de timeout pour les workloads critiques. |
...
# API server latency (average over a short period)
...
|--------|------------------------|----------------------------|
| **CNI** | Calico (BGP), Cilium (eBPF), Flannel (VXLAN) | **MTU** : Utiliser des jumbo frames pour les transferts de modèles volumineux et assurer une bande passante élevée entre les nœuds GPU. |
| **kube‑proxy** | iptables (mode `iptables`) ou IPVS. | IPVS offre un débit supérieur et une latence de connexion plus stable. Recommandé pour les services d’inférence à haute fréquence. |
...
metadata:
  name: dataset-pvc
spec:
  storageClassName: fast-ssd      # CSI provisioner
  accessModes:
  - ReadWriteOnce
  resources:
    requests:
...
|--------|--------------|--------------|--------------|--------------|
| TF ResNet‑50 (training, 10 ép.) | `tensorflow/tensorflow:2.13.0-gpu` |  |  |  |
