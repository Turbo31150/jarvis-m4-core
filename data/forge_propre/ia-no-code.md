...
**Objectif mesurable** : Auditer un scénario existant, réduire son temps moyen d’exécution et formaliser une politique de gouvernance des automatisations.  
...
|----------|-------------------|----------------|
| **Limite de requêtes par minute** (OpenAI) | Utiliser le **node “Throttle”** (n8n) ou le **“Sleep”** (Make) entre les appels. | ```json { "mode": "rate", "value": "<rate>", "timeUnit": "minute" }``` |
| **Burst autorisé** | Configurer un **bucket** avec une capacité et un taux de remplissage adaptés. | n8n → *Rate Limit* node (v0.225+) |
| **Back‑off exponentiel** en cas de 429 | Ajouter un **Retry** avec un nombre maximal de tentatives et un délai exponentiel. | n8n → *HTTP Request* → *Retry on Failure* avec `"maxAttempts": <maxAttempts>, "delay": "exponential"` |
...
|-----------|------------------|
| 5xx PDF.co (quota) | **Retry** : avec back‑off exponentiel |
| OpenAI renvoie `rate_limit_error` | **Router** → branche *fallback* → envoi d’un e‑mail d’alerte et mise en pause du scénario pendant une période définie |
| Airtable dépasse le taux d’écriture | **Throttle** : ajouter un module *Sleep* entre
---
...
```bash
# Linux/macOS
openssl rand -hex 32   # → chaîne hexadécimale, ex: a1b2c3...
```
...
| **Multipart/form‑data** | Upload de fichiers volumineux (limite > quelques mégaoctets) | `form-data` lib (Node) ou `requests` (Python) |
...
|-----------|------------|------------------------|
| **Longue exécution** | Retour immédiat `202 Accepted` + URL de polling | Le webhook renvoie `{ "job_id": "1234", "status_url": "https://my.api/job/1234" }`. Le client interroge `status_url` à intervalles réguliers. |
...
|------------|------------------------|----------------|------------|------------------|
| **Make**   | JavaScript (Node)      | Node 14 (LTS)  |            |                  |
| **Zapier** | JavaScript (Node)      | Node 14        |            |                  |
| **n8n**    | JavaScript (Node) **ou** Python | Node 18 / Python 3.9 |  |  |
...
- Le timeout inclut le temps d’initialisation du sandbox ; gardez le code léger.
...
- La taille du payload de retour est limitée. Si vous devez renvoyer de gros objets, sérialisez‑les en base64 ou stockez‑les dans un service externe (ex. S3) et renvoyez l’URL.  
- Le timeout est strict ; évitez les boucles lourdes ou les appels réseau (utilisez les modules HTTP natifs de Zapier à la place).
...
| **taux d’erreur** | `status` HTTP, `error` field du module | Dashboard d’erreurs, webhook `error` de Zapier | Un taux élevé indique besoin de retry ou de fallback. |
...
```js
// n8n Function node : batch prompts into a single OpenAI request
const BATCH_SIZE = /* taille du lot */;
...
Lorsque l’API impose une pagination (`limit/offset` ou `cursor`), récupérez le maximum d’items par appel. Implémentez un **loop** avec un compteur de page et arrêtez‑le dès que `has_more === false` ou que le nombre d’items atteint le besoin.
...
Pour les appels dont le résultat ne dépend que d’un identifiant (ex. `GET /documents/:id`), stockez la réponse dans un **key‑value store** (Redis, Airtable, Google Sheet) et réutilisez‑la pendant la durée de vie du cache (TTL = /* durée du cache */).  
...
|-----------|-----------|----------------|
| API limite à /* fréquence */ | **Token bucket** | n8n : `Wait` node + expression `Math.max(0, (Date.now() - $json.lastCall) - /* intervalle */)` |
