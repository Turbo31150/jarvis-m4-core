...
**Objectif mesurable** : Implémenter un pipeline de traitement de données depuis la capture sensorielle jusqu’à la génération d’un plan d’action dans un délai très court sur un jeu de données conséquent.  
**Notions couvertes**  
...
**Objectif mesurable** : Déployer et interroger un modèle de vision Transformer (ViT‑B/16) via l’API gRPC d’AlkymIA‑OS avec un taux de précision élevé sur le jeu de validation ImageNet‑100.  
**Notions couvertes**  
...
**Objectif mesurable** : Configurer un cluster Kubernetes 1.27 avec Helm 3.12 pour assurer une disponibilité très élevée des services critiques même sous une charge importante.  
**Notions couvertes**  
...
|------|-------------|------|-------------------|
| Ingestion | **Kafka** (topic `raw-events`) | Bufferisation durable, décorrélation producteur/consommateur | `acks=all`, `replication.factor=` |
| Transformation | **Kafka Streams** | Filtrage, normalisation, enrichissement léger (ex. conversion d’unités) | `state.dir` sur disque SSD, sérialisation **Avro** |
| Enrichissement | **Apache Flink** (JobManager + TaskManagers) | Jointure avec référentiels statiques, agrégation temporelle (tumbling windows) | `checkpointing.interval=`, `exactly‑once` |
| Persistance sémantique | **Blazegraph** (RDF 1.1) | Stockage des triplets, requêtes SPARQL | `http://host:9999/bigdata/namespace/kb/sparql` |
| Raisonnement | **Drools** (KIE‑API) | Application de règles déductives sur le graphe | Session Stateless, `fireAllRules()` |
| Cache de décision | **Caffeine** | Mémoire locale, TTL = , invalidation par évènement | `CacheWriter` qui supprime l’entrée dès qu’un nouveau triplet touche le même sujet |
| Monitoring | **Prometheus** + **Grafana** | Export des latences (`kafka_consumer_lag`, `flink_job_latency`, `drools_rule_time`) | `scrape_interval=` |
...
> **Pré‑requis** : JDK, Maven, Docker (Kafka, Zookeeper, Blazegraph, Prometheus).  
...
| **Conversion ONNX → TensorRT Engine** | ```bash<br># dans le container TensorRT 8.6 (Ubuntu 22.04)<br>trtexec --onnx=vit_b_16.onnx --saveEngine=vit_b_16.trt --fp16 --maxBatch=32 --workspace=4096<br>``` | `--fp16` réduit la latence d’inférence. `--workspace` limite la RAM GPU allouée. |
...
resources:
  ...
  requests:
    cpu: <demande CPU>
    memory: <demande mémoire>

autoscaling:
  enabled: true
  minReplicas: <minimum de réplicas>
  maxReplicas: <maximum de réplicas>
  targetCPUUtilizationPercentage: <pourcentage cible d’utilisation CPU>
  # métrique personnalisée (latence moyenne)
  customMetric:
    enabled: true
    name: latency_ms
...
Les métriques externes sont collectées à intervalles réguliers. Si l’intervalle de collecte est trop long, le scaling peut réagir avec un retard qui dépasse le SLA attendu. Solution : diminuer l’intervalle de collecte et activer `honor_labels: true` pour éviter la perte de tags.
...
* exécute les tests unitaires et d’intégration (coverage) ;  
...
* déclenche un déploiement Helm « canary » sur le cluster Kubernetes, puis, après validation de la santé, effectue le « roll‑out full ».  

Le tout doit être réalisable sur un runner GitHub Actions.  
...
| **Promotion** | Helm upgrade (values‑prod.yaml) | Service en production | `helm upgrade` après monitoring |
