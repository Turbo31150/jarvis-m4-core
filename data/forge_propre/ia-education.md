...
**Objectif mesurable** : Sélectionner, entraîner et évaluer un modèle de classification de texte capable de trier des questions d’élèves avec une précision satisfaisante sur un jeu de validation.
...
**Objectif mesurable** : Implémenter un système de recommandation qui propose plusieurs ressources pertinentes à un élève en fonction de son historique, avec un taux de clics prédit supérieur à la moyenne lors d’un test A/B.
...
**Objectif mesurable** : Construire un tableau de bord qui identifie les élèves présentant le plus fort risque d’échec, avec un rappel élevé sur un jeu de données historiques.
...
**Objectif mesurable** : Produire automatiquement un exercice à choix multiples (question, propositions, réponse correcte) à partir...
---
...
|--------|----------------------|------------------------|
| Régression logistique | Interprétable (coefficients), rapide sur jeux de taille modérée, probas calibrées | Linéarité → performances limitées si les relations sont non linéaires |
| Gradient Boosting (XGBoost, LightGBM) | Gère variables mixtes, robustesse aux valeurs manquantes, performances élevées sur données tabulaires | Risque d’over‑fitting, besoin de réglage d’hyper‑paramètres, moins transparent (requiert SHAP) |

Pour maximiser le vrai positif, on privilégie un modèle qui favorise le rappel.
...
def predict(features: StudentFeatures):
    ...
    X = np.array([[features.time_spent,
                   features.attempts,
                   features.sessions_last_week,
                   features.avg_score,
                   features.video_views]])
    # Faire la prédiction
    try:
        prediction = model.predict_proba(X)[:, 1].tolist()
    except Exception as e:
