...
|--------|------------|--------------------------|-------------------|
| **Leverage** | Multiplicateur appliqué au capital pour augmenter l’exposition. | Selon le contrat, le levier peut être ajusté. | <https://contract.mexc.com/api/v1/docs#leverage> |
...
| **Margin de maintenance** | Niveau minimum de marge requis pour garder la position ouverte. | Déterminé par le contrat. | Documentation MEXC – *Maintenance Margin* |
...
| **Risk per trade** | Portion du capital total risquée sur chaque trade. | Recommandé d’être faible selon les bonnes pratiques. | Best‑practice de la communauté des traders |

> **Règle d’or** : ne jamais risquer une part importante du capital total sur un trade, sauf si le portefeuille est dédié à du *high‑frequency* avec stop‑loss très serrés.
...
def compute_order_qty(
    ...
    risk_pct: float,         # proportion du capital à risquer
    entry_price: float,      # prix d’entrée du trade
    stop_price: float,       # prix du stop‑loss
    leverage: int = None,    # levier choisi
    tick_size: float = None, # incrément de prix du contrat
    min_qty: float = None    # volume minimal accepté par MEXC
) -> float:
    """
    ...
    # 5. Arrondi au tick de quantité (MEXC impose une précision décimale)
...
5. MEXC impose un volume minimal selon le contrat.  
6. Le `tick_size` du prix n’influence pas la quantité mais doit être respecté lors de la création de l’ordre (`price` arrondi).
...
def submit_order(
    ...
    """
    Soumet un ordre avec éventuellement un stop‑loss et un take‑profit.
    """
    order = {
        "symbol": symbol,
        "side": side,
    ...
    return _post("/order/submit", order)
```