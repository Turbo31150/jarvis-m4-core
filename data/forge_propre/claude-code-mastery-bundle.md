...
> Référence `claude-code-mastery-bundle` · prix mensuel  

## Plan

**Formation : « Prix mensuel » (référence claude-code-mastery-bundle, prix mensuel)**  

## Module 1 – Architecture des modèles de langage à grande échelle  
**Objectif mesurable :** L’apprenant pourra concevoir et documenter une architecture de modèle de langage de type Transformer contenant un nombre important de paramètres, en justifiant chaque choix d’hyper‑paramètre.  
**Notions couvertes**  
...
**Objectif mesurable :** L’apprenant sera capable de mettre en place une pipeline de tokenisation BPE ou SentencePiece adaptée à un corpus de grande taille, en évaluant un taux de couverture très élevé.  
**Notions couvertes**  
...
**Objectif mesurable :** L’apprenant pourra fine‑tuner un modèle pré‑entraîné sur une tâche de classification texte avec une amélioration notable du score F1 par rapport au baseline, en un temps d’entraînement raisonnable sur un serveur multi‑GPU.  
**Notions couvertes**  
...
**Objectif mesurable :** L’apprenant pourra déployer une API RESTful hébergeant le modèle fine‑tuné, capable de servir un volume élevé de requêtes avec une latence très basse, tout en appliquant le chiffrement TLS et le contrôle d’accès OAuth 2.0.  
**Notions couvertes**  
...
| **Couverture** | Proportion de caractères du corpus qui sont représentés par des tokens du vocabulaire. Calcul : `coverage = 1 - (OOV_characters / total_characters)`. | Méthode utilisée dans les rapports de HuggingFace Tokenizers |

---

## 2.2 Pipeline complet de tokenisation pour un corpus volumineux

### 2.2.1 Pré‑requis (Python version récente)
...
VOCAB_SIZE = 32000               # taille typique
...
|------|----------------|--------------|
| 3.1.1 | Choisir un jeu de données public déjà découpé en `train/validation` (ex. **IMDb** pour la classification sentiment). | `datasets.load_dataset("imdb")` renvoie deux splits avec un nombre égal d'exemples. |
...
| 3.1.4 | Créer la colonne `input_ids` via le tokenizer du modèle, en tronquant à la longueur maximale et en activant le `padding="max_length"`. | `tokenizer(..., truncation=True, padding="max_length", max_length=512)` produit des tensors de même taille. |
...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size=2,          # configuration du batch
    gradient_accumulation_steps=8,          # accumulation de gradients
    num_train_epochs=3,
    learning_rate=2e-5,
...
* **GPU mémoire** : chaque paramètre du modèle occupe plusieurs octets en FP32 et moins en FP16. Un modèle de cette taille nécessite une quantité de mémoire supérieure à celle d’un GPU standard, d’où la nécessité du **gradient accumulation** et de **ZeRO‑3** pour la production.
...
model.print_trainable_parameters()   # indique le nombre de paramètres entraînables
```

* **Avantage** : seules les petites matrices introduites par LoRA sont mises à jour, ce qui représente une très petite fraction du nombre total de paramètres, et donc la charge mémoire est largement réduite.  
...
# 2. Jeu de test (exemple : XSum – un sous‑ensemble)
...
*Commentaires*  
* `torch_dtype=torch.float16` réduit la consommation GPU sans perte de précision notable pour la génération.  
...
* **Typo aléatoire** : insérer, supprimer ou remplacer un caractère avec une petite probabilité.  
