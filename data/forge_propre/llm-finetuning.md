...
> Référence `llm-finetuning`

## Plan

## Module 1 : Principes fondamentaux du fine‑tuning d’un LLM  
**Objectif mesurable** : L’apprenant peut expliquer, à l’aide d’un diagramme, les différences entre le pré‑entraînement, le fine‑tuning complet et le fine‑tuning à paramètres faibles, et justifier le choix du dernier pour des modèles de grande taille.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant implémente une couche LoRA sur un modèle Hugging Face (ex. `bert-base-uncased`) et montre, à l’aide d’un benchmark, une réduction de la consommation GPU sans perte notable de précision sur la tâche GLUE MRPC.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant quantifie un modèle de grande taille en 4 bits avec la bibliothèque `bitsandbytes`, y ajoute LoRA, et obtient un score BLEU satisfaisant sur WMT‑14 En‑Fr avec un coût d’inférence réduit.  
**Notions couvertes**  
...
**Objectif mesurable** : L’apprenant configure un entraînement LoRA/QLoRA sur plusieurs GPU A100 via `accelerate` ou `deepspeed`, atteint une utilisation GPU élevée et complète plusieurs époques sur un dataset substantiel en un temps raisonnable.  
**Notions couvertes**  
...
Chaque bloc (attention + FFN) possède un nombre de poids entraînables proportionnel aux dimensions du modèle. La taille quadratique du produit `QKᵀ` impose une mémoire O(seq_len²).
...
|----------|----------------------|---------------------|-----------------------------|
| **Pré‑entraînement** | **Tous** (l'intégralité du modèle) | besoin de charger le modèle complet en FP16/FP32 → une fois la taille du modèle en VRAM | chaque itération coûte O(N_params) opérations de gradient |
| **Fine‑tuning complet** | **Tous** (souvent FP16) | même que pré‑entraînement, mais le dataset est plus petit → moins d’étapes, mais même besoin de VRAM | généralement plus lent que LoRA pour un même nombre d'étapes |
| **Fine‑tuning à paramètres faibles** | **une très petite proportion** (ex. LoRA) | seules les matrices additionnelles (`A,B`) sont stockées en FP16, le reste reste gelé en FP16/FP32 | mise à jour très rapide, le backward ne touche que les petites matrices, gradient computation ≈ O(r·d) au lieu de O(d²) |

> **Vérifiable** : dans le papier *LoRA* (Hu et al., 2021) les auteurs rapportent une très petite proportion de paramètres entraînables pour un grand modèle LLaMA avec une réduction notable de la consommation GPU sur un batch de taille modérée.

---

## 1.3 Méthodes de fine‑tuning à paramètres faibles  

| méthode | principe | où insérer les poids additionnels | nombre de paramètres (exemple) |
|--------|----------|----------------------------------|---------------------------------|
| **Adapters** | petite couche MLP (down‑proj + up‑proj) insérée après chaque FFN | `FFN_out → Down (d→r) → Up (r→d) → Add` | un petit nombre de paramètres, représentant une petite fraction du modèle |
| **Prefix‑tuning** | préfixe appris ajouté aux clés/valeurs de l’attention | `K,V ← concat([Prefix_K,V], K,V)` | un petit nombre de paramètres, représentant une petite fraction du modèle |
| **LoRA** | ajout d’une mise à jour de rang faible directement sur les matrices linéaires | `W ← W + α·(A·B)` où `A∈ℝ^{d×r}`, `B∈ℝ^{r×d}` | un petit nombre de paramètres, représentant une très petite fraction du modèle |
...
| métrique | fine‑tuning complet | LoRA (rang faible) | Adapters (rang modéré) |
|----------|----------------------|---------------------|------------------------|
| **Perplexité (GPT‑NeoX‑20B, WikiText‑103)** |  |  |  |
| **Temps d’inférence (batch = 1, seq_len = 1024)** |  |  |  |
| **Mémoire GPU (FP16)** |  |  |  |

> **Constat** : la perte de performance est généralement très faible pour de petits rangs, alors que la réduction de VRAM et le gain de vitesse d’entraînement sont significatifs.
---
...
base_model = AutoModelForSequenceClassification.from_pretrained(
    ...
    torch_dtype=torch.float16, # GPU optimisé FP16
)
...
|------|----------------|--------|-----------|--------------------------|
| **8‑bit (int8)** | 1 byte/paramètre | `torch.int8` (symmetric) | Gain notable sur la mémoire, support natif sur les GPU RTX 30 et A100 via `torch.cuda.amp` | Nécessite un calibrage de l’échelle (`scale`) sur chaque couche ; perte de précision légère sur la perplexité pour les LLM de grande taille. |
| **4‑bit (NF4)** | 0.5 byte/paramètre | `bitsandbytes.NF4` (non‑uniform) | Gain important, compatible avec les A100 40 GB et 80 GB, préserve les valeurs centrales du poids | Nécessite `bitsandbytes` ≥ 0.41, `torch.cuda.amp` désactivé pour les couches linéaires 4‑bit ; gradient approximatif (FP16) → possible instabilité si le taux d’apprentissage est trop élevé. |
| **4‑bit (FP4)** | 0.5 byte/paramètre | `bitsandbytes.FP4` (uniform) | Similaire à NF4 mais plus simple à implémenter | Perte de précision plus importante que NF4. |

**Pourquoi NF4 ?**  
- La distribution des poids d’un LLM pré‑entraîné suit une loi gaussienne centrée. NF4 utilise des points de quantisation non‑uniformes qui maximisent la densité autour de 0, réduisant l’erreur moyenne quadratique (MSE) de quantisation de façon significative par rapport à FP4.  
- Les travaux de Dettmers *et al.* (2023) montrent que NF4 maintient la performance sur les tâches de génération (BLEU, ROUGE) avec une perte négligeable par rapport à la version FP16.
...
- `A ∈ ℝ^{d×r}` et `B ∈ ℝ^{r×k}` sont apprises, avec `r` beaucoup plus petit que `d` et `k`.  
- Le poids effectif pendant le forward est `W + ΔW`.  
- `α` contrôle le facteur d’échelle.
...
```python
# Chargement du modèle en FP16 (pour le checkpoint d'origine)
...
# Quantisation NF4 (définit les Linear4bit à la place des Linear)
...
| **Gradient checkpointing** | Re‑calcule les activations intermédiaires pendant le backward au lieu de les stocker, réduisant la consommation mémoire de façon notable au prix d’un sur‑coût de calcul supplémentaire. | [Chen et al., 2016] |
...
offload_optimizer_device: cpu  # CPU off‑load, RAM recommandé
