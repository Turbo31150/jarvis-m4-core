...
> Référence `prompt-engineering`

## Plan

## Module 1 – Fondamentaux du Prompt Engineering

**Objectif d’apprentissage** : Être capable de concevoir, tester et itérer un prompt de base en respectant les critères de clarté, de concision et de contrôle de la température, avec un taux de réussite élevé sur un jeu de vingt requêtes ciblées.  
...
**Objectif d’apprentissage** : Générer automatiquement du code fonctionnel dans plusieurs langages (Python, JavaScript, Rust) à partir d’une description fonctionnelle, avec un taux de succès de compilation élevé sur trente exercices.  
...
**Objectif d’apprentissage** : Produire des transformations de données (JSON ↔ CSV, schémas SQL) à partir de prompts, avec une précision de parsing élevée sur un jeu de cinquante cas réels.  
...
**Objectif d’apprentissage** : Mettre en place un pipeline d’A/B testing automatisé pour comparer plusieurs variantes de prompts, et sélectionner la version offrant le meilleur score sur une tâche de classification texte.  
...
**Objectif d’apprentissage** : Déployer un service d’inférence capable de filtrer les prompts et les réponses selon une politique de sécurité définie, avec un taux de détection de contenus non conformes très élevé sur un ensemble de requêtes test.  
...
|--------|------|-----------------|
| **Instruction** | Action attendue du modèle (verbe d’ordre). | `Résume le texte suivant en quelques phrases.` |
| **Contexte** | Données ou informations nécessaires à la tâche. | `Texte : « … »` |
| **Contraintes** | Limites de forme, de style ou de longueur. | `Maximum de caractères, sans ponctuation finale.` |
...
Contexte : Le X200 possède un écran OLED, un processeur Snapdragon, et une batterie.
Contraintes : caractères max, inclure le hashtag #X200, ton enthousiaste, éviter les superlatifs non vérifiables.
```
...
| `top_p` (nucleus) | 0.8 – 1.0 (ex. 0.9) | Limite le vocabulaire aux tokens les plus probables selon le seuil choisi. |
...
def test_resume():
    prompt = (
        ...
        "avec un taux de croissance annuel depuis 2015.\n"
...
- `max_tokens` doit être supérieur à la longueur attendue + un petit buffer.  
