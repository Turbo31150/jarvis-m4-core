...
| Cache trop gros | Le répertoire de cache devient très volumineux | Sérialisation d’objets volumineux (ex. DataFrames) sans compression | Utiliser `@cache(compress=True)` ou stocker les artefacts intermédiaires dans un stockage d’objets (S3, MinIO). |
...
* `--no-cache-dir` empêche la création de caches inutiles dans l’image, ce qui réduit la taille de l’image.  
